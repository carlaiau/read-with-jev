...
export async function signIn(_state:AuthState,form:FormData):Promise<AuthState>{
 ...
 redirect(savedTextReturnPath(form.get('next'))??'/your-text');
}
export async function signUp(_state:AuthState,form:FormData):Promise<AuthState>{
 ...
 const next=savedTextReturnPath(form.get('next'));
 redirect(`/auth/sign-in?created=1${next?`&next=${encodeURIComponent(next)}`:''}`);
}
