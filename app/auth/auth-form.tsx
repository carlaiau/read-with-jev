...
export default function AuthForm({mode,created}:{mode:'in'|'up';created?:boolean}){
 ...
 return <main className="auth-page"><div className="auth-sheet">
  <Link className="text-sm text-muted underline underline-offset-4" href="/your-text">← Back to your text</Link>
  ...
  <p className="mt-8 text-sm text-muted">{mode==='in'?<>New here? <Link href="/auth/sign-up">Create an account</Link></>:<>Already have an account? <Link href="/auth/sign-in">Sign in</Link></>}</p>
 </div></main>;
}
