...
export default function YourTextReader({user,jevAvailable,requestedId=null,startNew=false}:{user:User;jevAvailable:boolean;requestedId?:string|null;startNew?:boolean}){
 ...
 <div className="custom-layout">
  <aside id="custom-sidebar" className={`custom-sidebar ${controlsOpen?'is-open':''}`} aria-label="Your texts and reading controls"><div className="custom-sidebar-content"><div className="custom-sidebar-intro"><Link className="custom-back-link" href="/">Go to book library</Link><h1>Your text</h1><p>Read your own writing with JEV</p></div><div className="custom-sidebar-heading"><h2>My texts</h2>{user&&<button type="button" onClick={()=>void newText()}>New text</button>}</div>
   ...
   <footer className="custom-sidebar-footer"><a className="text-xs text-muted underline underline-offset-4" href="https://github.com/carlaiau/readwithjev" target="_blank" rel="noreferrer">Source and research notes on GitHub</a></footer>
  </div></aside>
