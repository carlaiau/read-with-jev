...
import specs from '../library/documents.json';
...
for (const spec of specs as DocumentSpec[]) {
  ...
  const json = JSON.stringify(book); assert(Buffer.byteLength(json) < 5_500_000, 'Document exceeds function response budget');
