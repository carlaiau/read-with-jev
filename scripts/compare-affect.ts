...
import { makePairs, evaluatePairs, type AffectData, type EvaluationPair } from '../src/lib/affect';
import {highlightPairs} from '../src/lib/affect-highlights';
import { digest, readJson } from '../src/lib/io';

type ScoredPair = EvaluationPair & {documentId: string};
type Run = { threshold?:number; task: string; engine: string; prompt?: string; model: string | null; datasetHash: string; documentIds: string[]; scores: Record<string, number>; callsMade: number; elapsedMs: number; codeHashes?: Record<string, string>; responses: { cached: boolean; response: { model: string; usage?: { input_tokens: number; output_tokens: number } } }[] };
...
for (const r of runs) {
  assert.equal(r.task,runs[0].task,'Tasks differ');
...
const isHighlight=runs[0].task.startsWith('REMAN character-mention by eight emotions');
const pairs:ScoredPair[] = isHighlight ? documents.flatMap(highlightPairs) : documents.flatMap(makePairs);
const authors = [...new Set(documents.map(d => d.author))].sort();
const authorOf = new Map(documents.map(d => [d.doc_id, d.author]));
const subsets:Record<string,ScoredPair[]> = {strict:pairs};
if(!isHighlight){
 const relationPairs=documents.flatMap(makePairs);
 const linked=new Set(relationPairs.filter(p=>p.gold).map(p=>`${p.documentId}:${p.emotion.annotation_id}`));
 subsets.linkedExpressionsOnly=relationPairs.filter(p=>linked.has(`${p.documentId}:${p.emotion.annotation_id}`));
}
let seed = 20260919;
...
const label = (r: Run) => r.engine === 'jev' ? `jev-${r.prompt ?? 'v1'}${r.threshold&&r.threshold!==.5?'@'+r.threshold:''}` : r.engine;
const output = {
  task:runs[0].task, datasetHash: digest(raw), documentIds: runs[0].documentIds, documents: documents.length, authors: authors.length,
  uncertainty: '2000 paired author-cluster bootstrap resamples, seed 20260919, per-run fixed thresholds; development estimates, not held-out evidence',
  runs: runs.map((r,i) => ({ path: paths[i], label: label(r), requestedModel: r.model, threshold:r.threshold??.5, returnedModels: [...new Set(r.responses.map(x => x.response.model))],
    ...
    metrics: Object.fromEntries(Object.entries(subsets).map(([name, subset]) => [name, evaluatePairs(subset, Object.fromEntries(subset.map(p => [p.id, r.scores[p.id]])),r.threshold??.5)])),
  })),
...
for (const r of runs) evaluatePairs(pairs, r.scores,r.threshold??.5);
