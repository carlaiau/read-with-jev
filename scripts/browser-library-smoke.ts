...
try {
  ...
  await page.route('**/api/emotions?*',async route=>{const response=await route.fetch();const data=await response.json();await route.fulfill({json:typeof data==='string'?data:{...data,jevAvailable:false}});});
