...
try{
 ...
 await page.getByLabel('Emotion',{exact:true}).selectOption('trust');await page.waitForTimeout(350);assert.equal(calls.length,firstCalls,'Changing emotion must reuse scores');
 await page.locator('[data-jev-highlight]').first().click();await expect(page.getByLabel('Emotion inspection',{exact:true})).toBeVisible();await expect(page.getByLabel('Emotion inspection',{exact:true})).toBeFocused();await page.getByRole('button',{name:'Supported',exact:true}).click();await expect(page.getByText('Supported · saved on this device')).toBeVisible();
