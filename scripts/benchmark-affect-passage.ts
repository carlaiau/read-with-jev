...
const {values}=parseArgs({options:{sample:{type:'string'},cohort:{type:'string',default:'0'},execute:{type:'boolean',default:false},'cache-only':{type:'boolean',default:false},'max-requests':{type:'string',default:'0'}}});
...
const sample=values.sample?await readJson<{datasetHash:string;documentIds:string[]}>(values.sample):null;
if(sample)assert.equal(sample.datasetHash,digest(raw));
...
const docs=(sample?.documentIds??manifest.results[cohort].documentIds).map(id=>{const d=data.documents.find(d=>d.doc_id===id);assert(d&&d.split==='dev');assert.equal(sample?passageEligibility(d):highlightEligibility(d),null);return d;});
...
const plan={task:'REMAN direct passage eight emotions v1',prompt:'passage-v1',model:'jev-1.13.0',cohort:sample?null:cohort,...(sample?{samplePath:values.sample,sampleHash:digest(await readFile(values.sample!))}:{}),split:'dev',datasetHash:digest(raw),sourceHashes:data.sources,documentIds:docs.map(d=>d.doc_id),requests:jobs.length,nodeVersion:process.version,icuVersion:process.versions.icu};
if(!values.execute&&!values['cache-only'])console.log(JSON.stringify({...plan,status:'dry-run'},null,2));
 ...
 const path=`data/runs/affect-passage-${sample?'expansion':cohort}-${start}.json`;
